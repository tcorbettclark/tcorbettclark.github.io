...
**Note:** The HTML generated by the markdown conversion is the point of "maximum opinion" for TWG. It is the only aspect of the output which is not under the complete control of the author (in the sense that it contains HTML tags determined by the markdown library). Some of those tags need additional CSS to style (such as definition lists), and code or maths excerpts need even more substantial processing to look good. How that can be done is explained in my [content approach](approach.html).
